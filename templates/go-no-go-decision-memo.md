# Go / No-Go Decision Memo

Draft for educational and organizational purposes only. Review transaction decisions with qualified professionals before action.

## Decision summary

| Field | Value |
|---|---|
| Property / opportunity | |
| Strategy | |
| Date | |
| Current decision | go / conditional go / no-go / pause / learning only |

## User goal

- 

## Known facts

| Fact | Source | Confidence |
|---|---|---|
| | | |

## Assumptions

| Assumption | Why it matters | How to verify |
|---|---|---|
| | | |

## Financial view

| Item | Value |
|---|---:|
| Purchase price | |
| Estimated monthly payment | |
| Estimated taxes | |
| Estimated insurance | |
| Estimated repairs | |
| Estimated reserves needed | |
| Estimated cash needed | |

## Strategic fit

| Question | Answer |
|---|---|
| Does this fit the user's payment comfort zone? | |
| Does this fit the user's risk tolerance? | |
| Does this fit the timeline? | |
| Does financing appear realistic? | |
| Are professional reviews complete? | |

## Risks

| Risk | Severity | Mitigation |
|---|---|---|
| | | |

## Professional reviews

| Reviewer | Status | Notes |
|---|---|---|
| Lender | not started | |
| Real estate professional | not started | |
| Attorney | not started | |
| Title company | not started | |
| Inspector | not started | |
| Insurance | not started | |
| Tax professional | not started | |

## Decision

Choose one:

- Go
- Conditional go
- Pause
- No-go
- Learning only

## Reason

- 

## Next action

- 

## Russian learner explanation

Explain the decision to the user in Russian in practical, plain language.
