# Lender Comparison Template

Draft for educational and organizational purposes only. Final lending terms must come from the lender's official disclosures.

## Artifact metadata

| Field | Value |
|---|---|
| Artifact ID | |
| Artifact status | draft / updated / needs_review / final_for_user |
| Case ID | |
| Related lender IDs | |
| Date updated | |
| Primary next action | |

## Buyer summary

| Field | Value |
|---|---|
| Target area | |
| Intended use | |
| Purchase price assumption | |
| Down payment assumption | |
| Credit score range | |
| Income type | |
| Comfortable payment | |

## Lender comparison

| Lender | Program | Rate | APR | Points | Fees | Estimated payment | Estimated cash to close | Lock period | Notes |
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| | | | | | | | | | |

## Questions to ask lenders

- What loan program is this quote based on?
- What is the difference between the interest rate and APR here?
- Are points included?
- What fees are lender fees versus third-party fees?
- Is mortgage insurance included in the payment?
- What is the estimated cash to close?
- What documents are still needed?
- What could change before closing?
- Is the rate locked? If yes, until when?
- Are there state or local assistance programs that fit this buyer?

## Russian explanation notes

Use this section to translate the lender comparison into Russian for the learner.

## Open items

- 

## Operational notes

- Mark quotes as stale when the freshness window expires.
- Keep unresolved fee or rate questions as open tasks.
